...
public class EventLootTableLoad {
    ...
    public static void onLootTableLoad(LootTableLoadEvent event) {
        ...
        if (table.isFrozen()) {
            ...
            if (field == null) field = ReflectUtil.getField(table.getClass(), "field_186466_c"); // if null, try SRG name
            if (field == null) {
        ...
        for (LootPool pool : pools) {
            ...
            if (pool.isFrozen()) {
                ...
                if (field == null) field = ReflectUtil.getField(table.getClass(), "field_186453_a"); // if null, try SRG name
                if (field == null) {
            ...
            for (LootEntry entry : lootEntries) {
                if (entry instanceof LootEntryItem) {
                    ...
                    if (!item.getRegistryName().getNamespace().startsWith("minecraft")) {
                        pool.removeEntry(entry.getEntryName());
                        ATUtils.logger.info("Removed item({}) from pool({}/{})", item.getRegistryName(), event.getName(), pool.getName());
                    }
