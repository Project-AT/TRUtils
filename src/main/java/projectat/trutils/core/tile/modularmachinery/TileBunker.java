package projectat.trutils.core.tile.modularmachinery;
...
public class TileBunker extends TileColorableMachineComponent implements ITickable {
                ...
                for (int i = 0; i < inputBus.getInventory().getSlots(); i++) {
                    for (int n = 0; n < OUTPUT_SLOTS; n++) {
                        if (!outputInventory.getStackInSlot(n).isEmpty()) {
                            ItemStack stack = inputBus.getInventory().insertItem(i, new ItemStack(Main.equivalentFuel, 1), false);
                            if (stack.isEmpty()) {
    ...
    private void attemptStart() {
        ...
        for (int i = 0; i < INPUT_SLOTS; i++) {
            ...
            if (!stackInSlot.isEmpty() && outputInventory.getStackInSlot(OUTPUT_SLOTS - 1).getCount() != 64) {
                int fuel = getFuel(stackInSlot);
                this.burnTimeCache += fuel;
                if (burnTimeCache >= 200) {
                    if (insertOutput(new ItemStack(Main.equivalentFuel, getOutputAmount()))) {
        ...
        if (burnTimeCache >= 200 && !flag && outputInventory.getStackInSlot(OUTPUT_SLOTS - 1).getCount() != 64) {
            insertOutput(new ItemStack(Main.equivalentFuel, getOutputAmount()));
            markDirty();
        }
    }
    ...
    private int getFuel(ItemStack stack) {
        return stack.getItem() != Items.LAVA_BUCKET && stack.getItem() != Main.equivalentFuel ? TileEntityFurnace.getItemBurnTime(stack) : 0;
    }
    ...
    public int isWork() {
        for (int i = 0; i < INPUT_SLOTS; i++)
            if (!inputInventory.getStackInSlot(i).isEmpty() && outputInventory.getStackInSlot(OUTPUT_SLOTS - 1).getCount() != 64)
                return 1;
        return 0;
    }
