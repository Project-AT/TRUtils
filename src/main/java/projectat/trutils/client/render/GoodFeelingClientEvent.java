package projectat.trutils.client.render;

import crafttweaker.api.item.IIngredient;
import crafttweaker.api.minecraft.CraftTweakerMC;
import projectat.trutils.Main;
import projectat.trutils.core.events.GoodFeelingEvent;
...
public class GoodFeelingClientEvent {
        ...
        if (event.getType() == RenderGameOverlayEvent.ElementType.ALL) {
            ...
            if (pos != null) {
                ...
                if (PlayerHelper.hasHeldItemClass(mc.player, ItemTwigWand.class) &&
                    ...
                    profiler.startSection("trutils-wand");
    ...
    public static void renderHUD(Minecraft mc, ScaledResolution res, TileAlfPortal tile) {
        String name = I18n.format("item.trutils.good_feeling_level.name");
        int color = 0x4444FF;
...
//        net.minecraft.client.renderer.RenderHelper.disableStandardItemLighting();
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation(Main.MODID, "textures/items/good_feeling_level.png"));
//
//        x += 13;
        for (int i = 0; i < 5; i++) {
    ...
    public static void onItemTooltip(ItemTooltipEvent event) {
        for (Map.Entry<IIngredient, Double> entry : Main.ALF_PORTAL_EXPERIENCE.entrySet()) {
            if (entry.getKey().matches(CraftTweakerMC.getIItemStack(event.getItemStack()))) {
                if (entry.getValue() > 0)
                    event.getToolTip().add(I18n.format("item.trutils.alf_portal_experience.tooltip"));
                else
                    event.getToolTip().add(I18n.format("item.trutils.alf_portal_experience_.tooltip"));
            }
        }
    }
